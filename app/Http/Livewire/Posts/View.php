...
class View extends Component
{
    ...
    public function comments($post)
    {
        $post = Post::with(['comments.user' => function ($query) {
            $query->select('id', 'name', 'username','profile_photo_path', 'role_id');
        },
        ])->find($post);
        $this->postId = $post->id;
    ...
    public function createComment(Post $post)
    {
        ...
        session()->flash('comment.error', 'You are banned');
    }
            ...
            $posts = Post::withCount([
                            ...
                            'comments' => function ($query) {
                                $query->whereNotIn('user_id', function ($query) {
                                    $query->select('id')
                                          ->from('users')
                                          ->where('role_id', 3); // Assuming role_id 3 is for banned users
                                });
                            }
