<div class="flex flex-col sm:justify-center items-center pt-6 sm:pt-0 bg-gray-100">
    <div class="w-11/12 lg:w-full md:w-full sm:max-w-md mt-6 px-6 py-4 bg-white shadow-md overflow-hidden rounded-lg mb-12">
        ...
        <form wire:submit.prevent="submit" enctype="multipart/form-data">
            ...
            <div>
                ...
                <div x-data x-init="initializeMap()" wire:ignore id="mapCreateLocation" style="height: 400px; width: 100%;"></div>
                <x-jet-input disabled id="location" class="block mt-1 w-full" type="text" wire:model="location" />
            </div>
    ...
    <script wire:ignore>
        function initializeMap() {
            var map = new google.maps.Map(document.getElementById('mapCreateLocation'), {
                center: { lat: 0, lng: 0 },
                zoom: 1
            });
            ...
            map.addListener('click', function (event) {
                ...
                @this.set('location', clickedLatLng.lat() + ',' + clickedLatLng.lng());
            });
        }
    </script>
    ...
    <script>
        document.addEventListener('livewire:load', function () {
            Livewire.hook('message.sent', () => {
                Livewire.emit('locationUpdated', document.getElementById('location').value);
            });
        });
    </script>
@endpush